...
sub uninstall_content_caf {
  ...
  my $rootdir = shift;
  my @paths = ();
  if (defined(db_get_answer_if_exists('CAFVARDIR'))) {
      push @paths, db_get_answer('CAFVARDIR') . "/vmware-caf";
  }
  if (defined(db_get_answer_if_exists('CAFETCDIR'))) {
      push @paths, db_get_answer('CAFETCDIR');
  }
  push @paths, "/var/log/vmware-caf";
  push @paths, "/usr/lib/vmware-caf";

  foreach my $path (@paths) {
    uninstall_content_caf_process_files($path);
    if (-d $path) {
      rmdir($path);
    }
  }
...
sub install_content_tools {
  ...
  if ($open_vm_compat == 0) {
    ## Cleanup previous CAF files left over during upgrade
...
sub uninstall {
  ...
  # Remove CAF related files if any exist in the system.
  if (vmware_product() eq 'tools-for-linux') {
    if (!$open_vm_compat) {
      uninstall_content_caf();
    }
...
#
# code to run after the uninstaller has run
#

sub postuninstall_installed_tools {
  my $proxyDataDir = $gRegistryDir . '/GuestProxyData';
  system(shell_string($gHelper{'rm'}) . ' -rf ' . shell_string($proxyDataDir));
}
...
sub main {
   ...
   if (internal_basename($0) eq $cInstallerFileName) {
    ...
    if ($need_glibc25 eq 'yes' || $need_glibc211 eq 'yes') {
       my ($major, $minor) = get_glibc_version();
       if (! ($major > 2 || ($major == 2 &&  $minor >= 5))) {
          error("The glibc version detected ($major.$minor) is too old (< v2.5). " .
                $frozen_advice . "\n\n");
       } elsif (! ($major > 2 || ($major == 2 &&  $minor >= 11))) {
          error("The glibc version detected ($major.$minor) is too old (< v2.11). " .
                $frozen_advice . "\n\n");
       }
    }
  ...
  if (internal_basename($0) eq $cModuleUpdaterFileName) {
    ...
    if (not (($installed_version eq '10.3.25') and
  ...
  if (internal_basename($0) eq $gUninstallerFileName) {
    ...
    if ($#ARGV > -1) {
      @setOption = ();
      # There is currently one option:  --upgrade
      while ($#ARGV != -1) {
        my $arg;
        $arg = shift(@ARGV);
        if (lc($arg) =~ /^(-)?(-)?u(pgrade)?$/) {
