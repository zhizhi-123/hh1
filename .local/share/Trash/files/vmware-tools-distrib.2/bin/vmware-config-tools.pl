...
sub vmware_version {
  ...
  $buildNr = '10.3.25 build-20206839';
...
# Install a pam.d vmtoolsd file on all but Solaris.
  ...
  if (vmware_product() eq 'tools-for-linux') {
     ...
     if (-d '/sbin') {
       ...
       # If open_vm_compat mode, a version of the vmware-hgfsmounter may
       # not be installed by OVT packages, e.g. 10.2.5.  This installer
       # will add a symbolic link for /sbin/mount.vmhgfs be default and
       # in this case will incorrectly flag that symbolic link as having
       # "replaced" an OVT command.  To avoid this erroneous state,
       # check if the file pre-exists before any install logic.
       my $prevHgfsmnt = file_name_exist($sbinHgfsmntPath );
...
sub write_vmware_config {
  ...
  if (vmware_product() eq 'ws') {
    ...
    $config->set('product.version', '10.3.25');
  }
  $config->set('product.buildNumber', '20206839');
...
# Check if any modules are installed by VMware
sub have_modules_installed {
   return (keys %gVmwareInstalledModules) > 0;
}

# Check if any modules which need to be included in system ramdisk
# are installed by VMware
# return 1 if yes, 0 otherwise
sub have_ramdisk_modules_installed {
   foreach my $mod (@gRamdiskModules) {
      if (exists $gVmwareInstalledModules{$mod}) {
         return 1;
      }
   }
   return 0;
...
sub configure_tools {
  ...
  if (!$gOption{'modules_only'}) {
    ...
    if (vmware_product() eq 'tools-for-linux') {
      if (have_modules_installed()) {
        configure_auto_kmods();
      } else {
        print wrap("\nSkipping configuring automatic kernel modules as "
                   . "no drivers were installed by this installer.\n\n");
      }
      configure_ld_dot_so();
      if ($have_thinprint eq 'yes') {
        configure_thinprint();
  ...
  if (vmware_product() eq 'tools-for-linux') {
    if (have_ramdisk_modules_installed()) {
      configure_kernel_initrd();
    } else {
      print wrap("\nSkipping rebuilding initrd boot image for kernel as "
                 . "no drivers to be included in boot image were "
  ...
  # If open_vmt_compat mode, update the DB with kernel module status.
  if ($open_vm_compat == 1) {
    db_add_answer("OVT_VM_MODULES", have_modules_installed() ? "yes" : "no");
    db_add_answer("OVT_VM_RAMDISK", have_ramdisk_modules_installed() ?
                  "yes" : "no");
  }

  # If configuration has been called from the installer, do not emit the
  # "Enjoy" message here.   Let the install script emit the message.
  if (! $gOption{'installing'}) {
     print wrap('Enjoy,' . "\n\n" . '    --the VMware team' . "\n\n", 0);
  }
}
...
# Packaging requirements have changed with Solaris 11.4.  Determine
# if installing on Solaris release 11.4 or later.
# Returns 1 (true) if Solaris 11.4 or later; 0 (false) otherwise.
sub is_solaris_114_or_later {
  my $release = direct_command(shell_string($gHelper{'uname'}) . ' -r');
  my $version = getSolarisVersion();

  chomp($release);
  if (dot_version_compare("$release", '5.11') >= 0 &&
      dot_version_compare("$version", '11.4') >= 0) {
    return 1;
  }
  return 0;
}

# On Solaris, the SUNWuiu8 package has always been needed.  As of
# Solaris 11.4 (beta), that is no longer a requirement.  If the SUNWuiu8
# package is needed and not currently installed, terminate this
# configuration process.
sub verify_solaris_SUNuiu8_need {
  if (is_solaris_114_or_later() == 1) {
    return;
  }
...
sub verify_solaris_packages {
  ...
  if (is_solaris_114_or_later() == 1) {
    # Solaris 11.4 provides multiple versions of libpng.  Check for a
    # specific package - libpng14 (version 1.4.x)
    $pkg_name = "libpng14";
  }
  if ($pkg_cmd ne '') {
...
sub main {
   ...
   while ($#ARGV != -1) {
	 ...
	 #        Hence this is a moot argument.
         ...
         $gOption{'overwrite'} = 1;
      } elsif (lc($arg) =~ /^--installing$/) {
         $gOption{'installing'} = 1;
   ...
   if (vmware_product() eq 'tools-for-linux' ) {
     if ((defined db_get_answer_if_exists('OPEN_VM_COMPAT')) &&
         (db_get_answer('OPEN_VM_COMPAT') eq 'yes')) {
       $open_vm_compat = 1;
     }
   }
   ...
   # Be sure that the SUNWuiu8 package, if needed, is installed before
   # trying to configure.
   if (vmware_product() eq 'tools-for-solaris') {
     verify_solaris_SUNuiu8_need();
   }
